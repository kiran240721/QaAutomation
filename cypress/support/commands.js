...
Cypress.Commands.add('getCurrentDate', () => {
  ...
  Cypress.Commands.add('fillUserDetails', (user) => {
    ...
    if (user.autoPlanCreation) {
        cy.get('#IsInRoleSalesPerson').check();
      } else {
        cy.get('#IsInRoleSalesPerson').uncheck();
      }
    
      // Check the isActive checkbox if specified in user data
      if (user.isActive) {
        cy.get('#IsInRoleManager').check();
      } else {
        cy.get('#IsInRoleManager').uncheck();
      }
